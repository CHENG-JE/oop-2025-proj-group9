...
class Monster(pygame.sprite.Sprite):
        ...
        if self.state == 'idle':
            ...
            if self.attack_timer <= 0:
                if random.random() < 0.8:
                    self.state = 'beam_attack'
                    self.beam_count = 3
                    self.beam_cooldown = 0
                else:
                    self.state = 'shockwave_charge'
                    self.attack_timer = 60
        
        elif self.state == 'beam_attack':
            self.beam_cooldown -= 1
            if self.beam_cooldown <= 0 and self.beam_count > 0:
                beam = MonsterBeam(self.rect.center, player.rect.center)
                monster_projectile_group.add(beam)
                self.beam_count -= 1
                self.beam_cooldown = 42
            if self.beam_count <= 0:
                self.state = 'idle'
                self.attack_timer = 120

        elif self.state == 'shockwave_charge':
            self.attack_timer -= 1
            if self.attack_timer <= 0:
    ...
    def draw_health_bar(self, screen):
        ...
        title_rect = title_surface.get_rect(bottomleft=(bar_x, bar_y - 5))
